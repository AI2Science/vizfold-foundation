...
def _get_archive_reader(uploaded_file) -> ArchiveReader:
    """Cache backend ArchiveReader in session state, keyed by uploaded file hash."""
    ...
    if st.session_state.get("_zarr_hash") != file_hash:
        tmp_path = os.path.join(
            tempfile.gettempdir(),
            f"vizfold_uploaded_{file_hash}.zip",
        )
        ...
        st.session_state["_archive_reader"] = ArchiveReader(tmp_path)
...
with st.sidebar:
    ...
    # ── Branch B: Zarr archive ────────────────────────────────────────────────
        ...
        type=["zip"],
        help=(
            "Upload a zipped Zarr archive containing attention arrays, "
            "representations, metadata, and optional structure outputs."
        ),
    )
    ...
    with st.spinner("Opening Zarr archive..."):
        archive_reader = _get_archive_reader(uploaded)

    meta = archive_reader.metadata()

    with st.expander("Archive metadata", expanded=False):
        st.write("Archive kind:", meta.archive_kind)
        st.write("Model:", meta.model_family)
        st.write("Model version:", meta.model_version)
        st.write("Attention types:", meta.attention_types)
        st.write("Capabilities:", meta.capabilities)
        st.write("Available arrays:", meta.extras.get("arrays", {}))

    attention_types = archive_reader.list_attention_types()

    if not attention_types:
        st.error("No attention arrays found in this archive.")
        st.stop()

    attn_type = st.selectbox("Attention type", attention_types)

    layers = archive_reader.list_layers(attn_type)

    if not layers:
        st.error(f"No layers found for attention type `{attn_type}`.")
        st.stop()

    layer_idx = st.select_slider("Layer", options=layers, value=layers[-1])

    residue_options = archive_reader.list_residue_indices(attn_type, layer_idx)
    residue_idx = None
    ...
    if residue_options:
        residue_idx = st.select_slider(
            ...
            value=residue_options[0],
        )

    heads = archive_reader.list_heads(attn_type, layer_idx, residue_idx)

    if not heads:
        st.error(f"No heads found for `{attn_type}` layer {layer_idx}.")
        st.stop()
    ...
    if head_sel_zarr == "Average":
        loaded_heads = archive_reader.load_attention_heads(
        ...
        connections = []
        for attention_slice in loaded_heads.values():
            connections.extend(attention_slice.as_triplets())

        connections = sorted(connections, key=lambda x: x[2], reverse=True)
        head_label = "All heads averaged"

    else:
        attention_slice = archive_reader.load_attention(
            attention_type=attn_type,
            layer=layer_idx,
            head=int(head_sel_zarr),
            residue_idx=residue_idx,
            top_k=top_k,
        )

        connections = attention_slice.as_triplets()
        head_label = f"Head {head_sel_zarr}"

        structure_data = archive_reader.load_structure()

        pdb_path = None
        pdb_string = structure_data.pdb_text

        n_residues = (
            meta.extras.get("num_residues")
            or len(structure_data.sequence or "")
            or len(meta.sequence or "")
        )

        if not n_residues and connections:
            n_residues = max(max(src, dst) for src, dst, _ in connections) + 1

        fasta_seq = structure_data.sequence or meta.sequence or ("X" * int(n_residues))
        protein = meta.protein_id
        attn_badge = (
            f"{attn_type}"
            if residue_idx is None
            else f"{attn_type} (res {residue_idx})"
        )

        st.caption(
            f"{n_residues} residues · {len(layers)} layers · {len(heads)} heads"
        )
