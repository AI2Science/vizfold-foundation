...
> Solution: Something went wrong with setting up some of the custom kernels. Try `pip install --no-build-isolation .` from inside the OpenFold folder. Make sure to prepend the conda environment as described above before running this.
