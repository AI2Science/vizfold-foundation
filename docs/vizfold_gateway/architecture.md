...
`RUN` is one concrete execution request. It selects a model backend, execution target, and invocation profile, then records the concrete model and runtime choices for that run, the sequence folded (`input_sequence`), and an immutable `provenance_json` snapshot of the backend/target/profile and resolved install paths as they were at queue time. `RUN.input_id` is the stable model-facing input identity: for a batch it is the folded tags joined with `+`, and for one target it is that tag. For OpenFold this is enforced: preflight fails a run whose FASTA header tags are not exactly the tags `input_id` names, and the precomputed-alignment key is `alignment_dir/<tag>` for each of them. It should not be mutated for workspace/output collision handling; use run/workspace identifiers for that instead.

`ARTIFACT_TYPE` is catalog/reference data for the kinds a folding run produces: protein structures, attention maps and their dense tensors, activation tensors, the model output archive, the trace index and summary, run metadata and logs, the sequence alignments and template hits a run searches for itself, and a catch-all for anything else it wrote. It stores a slug, default format, display mode, viewer kind, description, and an optional metadata schema, separating the classification and the hints for showing a kind from the produced instances. The catalog and the rule that classifies a file are the same table — `cli/src/core/artifact_kinds.rs` — so a kind cannot exist in one and not the other.
...
Post-run artifact discovery runs inline: a run that exits 0 walks its output directory and registers every file it produced under the kind its path says it is, through `services::run_artifacts::register_known_run_artifacts`, idempotently; `vizfold register-artifacts <id>` re-runs it and reports what each kind holds. Classification is deterministic from the path alone (`artifact_kinds::classify`), so no backend declares what it wrote, and an instance carries what its name encodes — target, layer, attention type, timing stage — in its `metadata_json`. Re-registering after the classifier learns a new kind moves an existing row onto it rather than leaving the file typed by an older rule. The workbench (`vizfold serve`) reads those rows and takes the viewer and display mode from the kind rather than guessing from the file extension.
